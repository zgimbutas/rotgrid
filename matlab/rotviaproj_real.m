...
if( nargin == 3 ),
mex_id_ = 'rotviaproj_realf90(i double[x], i int[x], i int[x], i int[x], i dcomplex[], i int[x], io dcomplex[], i int[x])';
[mpout] = rotgrid_r2014a(mex_id_, beta, nterms, m1, m2, mpole, lmp, mpout, lmpn, 1, 1, 1, 1, 1, 1);
end

if( nargin >= 4 ),
mex_id_ = 'rotviaproj2_realf90(i double[x], i double[x], i int[x], i int[x], i int[x], i dcomplex[], i int[x], io dcomplex[], i int[x])';
[mpout] = rotgrid_r2014a(mex_id_, beta, alpha, nterms, m1, m2, mpole, lmp, mpout, lmpn, 1, 1, 1, 1, 1, 1, 1);
end
