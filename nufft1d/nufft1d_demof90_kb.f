...
c
      integer i,ier,iflag,j,k1,mx,ms,nj
        parameter (mx=1 000 000)
      real*8 xj(mx), sk(mx)
        real*8 err,eps,pi,t1,t2,second
      parameter (pi=3.141592653589793238462643383279502884197d0)
      complex*16 cj(mx),cj0(mx),cj1(mx)
      complex*16 fk0(mx),fk1(mx)

      real *8 fm(mx), xc(15*mx)
c
c     --------------------------------------------------
...
c     -----------------------
c
ccc         call dirft1d1(nj,xj,cj,iflag, ms,fk0)
         
         t1=second()
         call nufft1d_kb_param(ms,eps,nf1,beta,nspread)
         call nufft1d_kb_fm_init(nf1,beta,nspread,ms,fm)
         call nufft1d_kb_xc_init(nf1,beta,nspread,nj,xj,xc)
         t2=second()
         write(*,*) 'init=*',t2-t1
         ...
         t1=second()
         call nufft1d1f90_kb(nj,xj,cj,iflag,eps, ms,fk1,ier,
     $      nf1,nspread,xc,fm)
         t2=second()
         write(*,*) 'time=*',t2-t1
...
ccc         call dirft1d2(nj,xj,cj0,iflag, ms,fk0,ier)
         ...
         t1=second()
         call nufft1d2f90_kb(nj,xj,cj1,iflag,eps, ms,fk0,ier,
     $      nf1,nspread,xc,fm)
         t2=second()
         write(*,*) 'time=*',t2-t1
