...
%j=1;
%for i=0:nterms
%mpole(i+1,j+(0:i)) = 1/sqrt(2*i+1); 
%end

j=1;
for i=0:nterms
mpole(i+1,j+(0:i)) = rand(1,1);
...
[ctheta,whts,ynms,wsave]=sphtrans_real_cheb_init(nterms,nphi,ntheta);
...
[rotmat]=rotgrid_fsr_real_init(nterms,nbeta,beta);

tic
grids=rotgrid_dsr_real(nterms,mpole,nphi,ntheta,nbeta,beta,nrot,...
