...
c
c     rotviarecur3_real is an f77 routine (needs a workspace).
c     rotviarecur3f90_real is an f90 routine (no workspace needed).
c
c     rotviarecur3p_init_real  - precompute the coefficients of rotation matrix.
c     rotviarecur3p_apply_real - apply the rotation matrix.
c
c
C*****************************************************************
        subroutine rotviarecur3_real
     $     (theta,nterms,m1,m2,mpole,ld1,marray,ld2,w,lw,lused)
C*****************************************************************
c
c       Purpose:
c
...
c
c       Note that this is just a driver to rotviarecur3s_real and
c       rotviarecur3t_real routines, the main purpose of this routine is to
...
c
        ...
        call rotviarecur3t_real(nterms,m1,m2,mpole,ld1,marray,
     1                    ld2,w(ird1),w(ird2),w(isqc),theta,ldc)
        else
        call rotviarecur3s_real(nterms,m1,m2,mpole,ld1,marray,
     1                    ld2,w(ird1),w(ird2),w(isqc),theta,ldc)
        endif
c
        return
...
c
C*****************************************************************
        subroutine rotviarecur3t_real(nterms,m1,m2,mpole,ld1,marray,
...
c
	...
	hsthta=ww*stheta
cc	cthtap=+ww*(done+ctheta)
cc	cthtan=-ww*(done-ctheta)
...
c
cc	    do im=-ij,-imp
cc	       rd2(imp,im)=rd1(imp-1,im+1)*cthtan*sqc(ij-im,2)
cc	       if (im.gt.(1-ij)) then
cc	          rd2(imp,im)=rd2(imp,im)-
cc     1            rd1(imp-1,im-1)*cthtap*sqc(ij+im,2)
cc	       endif
cc	       if (im.gt.-ij) then
cc	          rd2(imp,im)=rd2(imp,im)+
cc     1            rd1(imp-1,im)*stheta*sqc(ij+im,1)*sqc(ij-im,1)
cc	       endif
cc	       rd2(imp,im)=rd2(imp,im)*ww/sqc(ij+imp,2)
ccc
cc	       if (im.lt.-imp) then
cc               if( mod(imp-im,2) .eq. 0 ) then
cc               rd2(-im,-imp)=+rd2(imp,im)
cc               else
cc               rd2(-im,-imp)=-rd2(imp,im)
cc               endif
cc               endif
...
ccc
cc	    do im=imp,ij
cc	       rd2(imp,im)=rd1(imp-1,im-1)*cthtap*sqc(ij+im,2)
cc	       if (im.lt.(ij-1)) then
cc	          rd2(imp,im)=rd2(imp,im)-
cc     1            rd1(imp-1,im+1)*cthtan*sqc(ij-im,2)
cc	       endif
cc	       if (im.lt.ij) then
cc	          rd2(imp,im)=rd2(imp,im)+
cc     1           rd1(imp-1,im)*stheta*sqc(ij+im,1)*sqc(ij-im,1)
...
c
C*****************************************************************
        subroutine rotviarecur3s_real(nterms,m1,m2,mpole,ld1,marray,
     1                         ld2,rd1,rd2,sqc,theta,ldc)
C*****************************************************************
c
c       Purpose:
...
c
C*****************************************************************
        subroutine rotviarecur3x_real(nterms,m1,m2,mpole,ld1,marray,
     1                         ld2,rd1,rd2,sqc,theta,ldc)
C*****************************************************************
c
...
c
C*****************************************************************
        subroutine rotviarecur3f90_real
     $     (theta,nterms,m1,m2,mpole,ld1,marray,ld2)
C*****************************************************************
c
c       Purpose:
c
...
c
C*****************************************************************
        subroutine rotviarecur3p_init_real(ier,rotmat,ldc,theta)
C*****************************************************************
c
c       Purpose:
c
...
c
C*****************************************************************
        subroutine rotviarecur3p_apply_real
...
c
     ...
     1              dconjg(mpole(ij,mp))*rotmat(mp,ij,-m)
                 enddo
              enddo
           enddo
        else
