...
c
c     User-callable f77 routine is rotviaproj3. 
c     User-callable f90 routine is rotviaproj3f90.
c     The other routines are used internally.
c
c***********************************************************************
      subroutine rotviaproj3(beta,nrot,nterms,m1,m2,mpole,lmp,
     $     marray2,lmpn,w,lw,lused)
c***********************************************************************
c       Purpose:
c
c	Fast and stable algorithm for applying rotation operators about
...
c       nrot:  the number of rotation angles about the z-axis.
...
c
      ...
      complex *16 marray2(0:lmpn,-lmpn:lmpn,nrot)
c
c      nquad = 2*nterms+2
c        write(*,*) nquad
...
c        write(*,*) nquad
      ...
      iuder = iuval + 2*nquad*(nterms+1)*nrot
      iephi = iuder + 2*nquad*(nterms+1)*nrot
      iwsave = iephi + 2*(2*nterms+1)
      iavec = iwsave + 4*max(nquad,nrot)+20
      ibvec = iavec + 2*max(nquad,nrot)
      lused = ibvec + 2*max(nquad,nrot)
      if (lused.gt.lw) stop
c
      call rotviaproj30(beta,nrot,nquad,nterms,m1,m2,mpole,lmp,
...
c
c***********************************************************************
      subroutine rotviaproj30(beta,nrot,nquad,nterms,m1,m2,mpole,lmp,
...
c
      ...
      complex *16 uder(nquad,nrot,0:nterms),uval(nquad,nrot,0:nterms)
...
c
      ...
      call evalall0b(beta,nrot,nquad,cthetas,sthetas,cphis,sphis,mpole,
     2           lmp,nterms,uval,uder,ynm,ynmd,ephis,rat1,rat2)
      call projectonynm2b(nquad,nrot,
...
C
C***********************************************************************
      subroutine evalall0b(beta,nrot,nquad,cthetas,sthetas,cphis,sphis,
...
c
	 do ndeg = 0,nterms
	    ...
	    do morder = -ndeg,ndeg
            ...
            irot=morder+1
            if( morder .lt. 0 ) irot=nrot+morder+1
c
c       ... map irot into range [1,nrot] 
            if( irot .le. 0 ) irot = irot + 2*nterms*nrot
            if( irot .gt. nrot ) irot = mod(irot,nrot)
            if( irot .eq. 0 ) irot = nrot
c
c       ... apply the periodizing operator
c            with aliasing, if nrot < 2*nterms+1
c
...
C
C***********************************************************************
      subroutine projectonynm2b(nquad,nrot,uval,uder,
...
C***********************************************************************
      ...
      complex *16 ephi,ephi1,uval(nquad,nrot,0:nterms)
      complex *16 uder(nquad,nrot,0:nterms)
      complex *16 utheta,uphi,ztmp1,ztmp2
      complex *16 alpha,beta,ima
      complex *16 marray(0:lmpn,-lmpn:lmpn,nrot)
      real *8 ynm(0:nterms,0:nterms)
      real *8 ynmd(0:nterms,0:nterms)
      real *8 wsave(*)
      complex *16 avec(*)
...
c
      ...
      do ii = 1,nquad
        do irot=1,nrot
	    avec(irot) = uval(ii,irot,norder)*d+uder(ii,irot,norder)
        enddo
        call zfftb(nrot,avec,wsave)
        do irot=1,nrot
	    uval(ii,irot,norder) = avec(irot)
        enddo        
      enddo
      enddo
...
c
c***********************************************************************
      subroutine rotviaproj3f90(beta,nrot,nterms,m1,m2,mpole,lmp,
     1           marray2,lmpn)
c***********************************************************************
c       Purpose:
c
...
c       OUTPUT:
c
c       marray2  coefficients of rotated expansions.
c
C---------------------------------------------------------------------
c
c
c
      implicit none
      integer nrot,nquad,ier,m1,m2,nterms,lmp,lmpn, next235_cproj
...
c        write(*,*) nquad
      ...
      lused = iwsave + 4*max(nquad,nrot)+20
      allocate (w(lused), stat=ier)
      iuval = 1
      iuder = iuval + nquad*(nterms+1)*nrot
      iephi = iuder + nquad*(nterms+1)*nrot
      iavec = iephi + (2*nterms+1)
      ibvec = iavec + 2*max(nquad,nrot)
      lused = ibvec + 2*max(nquad,nrot)
      allocate (cw(lused), stat=ier)
      if (ier.ne.0) then 
         write(6,*) ' alloc failure in rotviaproj3f90'
         stop
      endif
c
      call rotviaproj30(beta,nrot,nquad,nterms,nterms,nterms,
