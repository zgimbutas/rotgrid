...
c
c       rotviarecur3pa_apply - apply the rotation matrix, move pole to 
c                              location (beta, alpha)
c       rotviarecur3pf_apply - apply the rotation matrix, move pole to 
c                        locations (beta, alpha_k), alpha_k = 2*pi *(k-1)/nrot
c
C*****************************************************************
        subroutine rotviarecur3pa_apply
     $     (beta,alpha,nterms,m1,m2,mpole,ld1,marray,ld2,rotmat,ldc)
C*****************************************************************
c
c       Purpose:
c
...
c       location (beta, alpha) in spherical coordinates.
c
C---------------------------------------------------------------------
c       INPUT:
c
...
c
     ...
     1              mpole(ij,-mp)*dconjg(cd)*rotmat(mp,ij,-m)
                 enddo
              enddo
           enddo
        else
...
c
C*****************************************************************
        subroutine rotviarecur3px_apply
...
c
C*****************************************************************
        subroutine rotviarecur3pf_apply
     $     (beta,nrot,nterms,
...
c       locations (beta, alpha_k) in spherical coordinates (beta, phi),
...
ccc	implicit none
	...
	real *8 beta,alpha,pi,done
	complex *16 mpole(0:ld1,-ld1:ld1)
	complex *16 marray(0:ld2,-ld2:ld2,nrot)
	real *8 rotmat(0:ldc,0:ldc,-ldc:ldc)
...
c
        ...
        w(mp2) = w(mp2)+mpole(ij,-mp)*rotmat(mp,ij,-m) 
        mp1=mp1+1
...
c
        ...
        do mp=1,ij          
          ...
          w(nrot-mp+1) = mpole(ij,-mp)*rotmat(mp,ij,-m) 
        enddo
        call zfftb(nrot,w,wsave)
...
c
     ...
     1       mpole(ij,-mp)*dconjg(cd)*rotmat(ij,-m,mp)
          enddo
        enddo
...
c
        ...
        w(mp2) = w(mp2)+mpole(ij,-mp)*rotmat(ij,-m,mp) 
        mp1=mp1+1
        if( mp1 .gt. nrot ) mp1=1
        mp2=mp2-1
        if( mp2 .lt. 1 ) mp2=nrot
...
c
        ...
        do mp=1,ij          
          ...
          w(nrot-mp+1) = mpole(ij,-mp)*rotmat(ij,-m,mp) 
        enddo
        call zfftb(nrot,w,wsave)
